...
# =============================================================================
''' 
Our sequence-to-sequence Transformer consists of a TransformerEncoder and a 
TransformerDecoder chained together. To make the model aware of word order, 
we also use a PositionalEmbedding layer.

The source sequence will be passed to the TransformerEncoder, which will 
produce a new representation of it. This new representation will then 
be passed to the TransformerDecoder, together with the target sequence
so far (target words 0 to N). The TransformerDecoder will then seek 
to predict the next words in the target sequence (N+1 and beyond).

A key detail that makes this possible is causal masking
(see method get_causal_attention_mask() on the TransformerDecoder).
The TransformerDecoder sees the entire sequences at once, 
and thus we must make sure that it only uses information
from target tokens 0 to N when predicting token N+1
(otherwise, it could use information from the future, 
which would result in a model that cannot be used 
at inference time). '''
...
we should train for at least 30 epochs.
"""
...
max_decoded_sentence_length = 20  # This is the maximum length of the decoded sentence. It is also the length of the output sentence. This is to be able to generate sentences of the same length as the input sentences.
...
def decode_sequence(input_sentence):
    ...
    for i in range(max_decoded_sentence_length):
        ...
        if sampled_token == "[end]":
            break
    return decoded_sentence.replace("[start]", "").replace("[end]", "")  # Remove the '[start]' and '[end]' tokens.

test_eng_texts = [line.rstrip('\n') for line in open("test_eng.txt")]  # Read and store the test texts. This is not the test set. This is just a set of sentences to translate to Spanish.
for _ in range(30):
    input_sentence = random.choice(test_eng_texts)  # Pick a random sentence from the test set. This is the input sentence.
    translated = decode_sequence(input_sentence)  # Translate the input sentence.  
     '''
...
while True:
    input_sentence = input("Enter an English sentence to translate to Spanish: ")
